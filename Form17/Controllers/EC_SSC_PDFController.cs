...
{
    public class EC_SSC_PDFController : Controller
    {
        // GET: EC_SSC_PDF
        public void Create_SSC_EC_PDF(EC_HSC_ModelController ec)
        {
          
            var date = DateTime.Now.ToString("d/M/yyyy");
            EC_HSC_PDFController eC_HSC_PDFController = new EC_HSC_PDFController();
            Document document = new Document(PageSize.A4, 25, 25, 30, 30);

            Random r = new Random();

            PdfWriter.GetInstance(document, new System.IO.FileStream(System.IO.Path.Combine(HttpRuntime.AppDomainAppPath, "PDF/SSC_EC/" + ec.S_ID + ".pdf"), FileMode.Create));
            ...
            phars1.Add(new Chunk("Certified that the candidate whose particulars are mentioned below is permitted to appear as a PRIVATE CANDIDATE for the regular S.S.C Examination of", fdefault12));
            phars1.Add(new Chunk(" Year " + ec.HSC_Year + " ", fontb));
            phars1.Add(new Chunk("or any subsequent S.S.C Examination, Subject to rules governing admission of Private Candidate's to the Examination,through the recognised High School", fdefault12));
            ...
            iTextSharp.text.Image img1 = iTextSharp.text.Image.GetInstance(System.IO.Path.Combine(HttpRuntime.AppDomainAppPath, "AppFiles/Images/SSC/" + ec.Ref_ID + "/Photo.jpeg"));
            img1.Alignment = iTextSharp.text.Image.ALIGN_CENTER;
            img1.ScaleAbsolute(90.0f, 70.0f);
            ...
            PdfPCell cellop4 = new PdfPCell(new Phrase("SCHOOL NO", fdefault12));
            cellop3.PaddingBottom = 5;
            ...
            PdfPCell Notes2 = new PdfPCell(new Phrase("Notes: \n\n1) Student must submit examination form for S.S.C examination from above mentioned school only. \n\n2)This Certificate Must be preserved and EC Number must be entered on candidate's examination application form for admission to the examination.\n\n3) This Certificate shall be valid for admission to the S.S.C Examination held in March or July of any year unless otherwise notified.", Notes));
            ...
            iTextSharp.text.Image Sign = iTextSharp.text.Image.GetInstance(System.IO.Path.Combine(HttpRuntime.AppDomainAppPath, eC_HSC_PDFController.GetSignaturePath(ec.Division)));
            Sign.ScaleAbsolute(80.0f, 70.0f);
            var ph = new Phrase();
            ph.Add(new Chunk("\r"));
